...
const QRScanner: React.FC = () => {
  ...
  const handleCheckOut = async (qrCode: string) => {
    ...
    try {
      ...
      if (response.new_badges && Array.isArray(response.new_badges) && response.new_badges.length > 0) {
        msg += ` 🏆 New badges earned: ${response.new_badges.map(b => b.name).join(', ')}`;
      }
      setMessage(msg);
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-10 pb-8 flex flex-col justify-center items-center w-full">
